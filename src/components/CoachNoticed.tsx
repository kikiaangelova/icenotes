...
/**
 * Personalized "Noticed in your week…" panel for the dashboard.
...
export const CoachNoticed: React.FC<CoachNoticedProps> = ({ onOpenReflect }) => {
  ...
  // Default Noticed in your week for steady weeks
  if (insights.length === 0 && signals.entriesLast7 >= 2) {
    insights.push({
      key: 'coach',
      icon: <Sparkles className="w-4 h-4" />,
      tone: 'coach',
      title: bg ? 'Забелязано тази седмица…' : 'Noticed in your week…',
      body: bg
  ...
  return (
    <section
      aria-label={bg ? 'Забелязано тази седмица' : 'Noticed in your week'}
    ...
    >
      <div className="flex items-center gap-2 px-1">
        ...
        <h2 className="text-[11px] font-bold tracking-[0.18em] uppercase text-muted-foreground">
          {bg ? 'Забелязано тази седмица' : 'Noticed in your week'}
        </h2>
      </div>
