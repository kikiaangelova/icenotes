...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({ active, onChange }) => {
  ...
  return (
    ...
    >
      <ul className="max-w-2xl mx-auto grid grid-cols-6 gap-0.5 px-1.5 pt-2 pb-2">
        {ITEMS.map((item) => {
          ...
          return (
            <li key={item.id}>
              <button
                ...
                className={cn(
                  'relative w-full min-h-[60px] flex flex-col items-center justify-center gap-1 rounded-2xl motion-press transition-all duration-300 ease-out touch-manipulation px-0.5',
                  isActive
