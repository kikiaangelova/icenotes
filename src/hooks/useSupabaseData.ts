  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...
        // Athlete context (Phase 2). Height/weight are deliberately NOT read or used.
        skatingCategory: (data as { skating_category?: string }).skating_category || undefined,
        yearsSkating: (data as { years_skating?: number }).years_skating ?? undefined,
        currentElements: (data as { current_elements?: string }).current_elements || undefined,
        biggestChallenge: (data as { biggest_challenge?: string }).biggest_challenge || undefined,
        nextCompetition: (data as { next_competition?: string }).next_competition || undefined,
        nextCompetitionDate: (data as { next_competition_date?: string }).next_competition_date || undefined,
        supportAreas: (data as { support_areas?: string[] }).support_areas || undefined,
        supportStyle: (data as { support_style?: string }).support_style || undefined,
        usefulNote: (data as { useful_note?: string }).useful_note || undefined,
        avatarUrl: data.avatar_url || undefined,
        language: ((data as { language?: string }).language as SkaterProfile['language']) || undefined,
    ...
    mutationFn: async (profile: Partial<SkaterProfile>) => {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          ...(profile.skatingCategory !== undefined ? { skating_category: profile.skatingCategory } : {}),
          ...(profile.yearsSkating !== undefined ? { years_skating: profile.yearsSkating } : {}),
          ...(profile.currentElements !== undefined ? { current_elements: profile.currentElements } : {}),
          ...(profile.biggestChallenge !== undefined ? { biggest_challenge: profile.biggestChallenge } : {}),
          ...(profile.nextCompetition !== undefined ? { next_competition: profile.nextCompetition } : {}),
          ...(profile.nextCompetitionDate !== undefined ? { next_competition_date: profile.nextCompetitionDate || null } : {}),
          ...(profile.supportAreas !== undefined ? { support_areas: profile.supportAreas } : {}),
          ...(profile.supportStyle !== undefined ? { support_style: profile.supportStyle } : {}),
          ...(profile.usefulNote !== undefined ? { useful_note: profile.usefulNote } : {}),
